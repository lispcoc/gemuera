...
internal sealed class Utils
{
	...
	public static void AddColorParam(StringBuilder sb, string name, EraColor EraColor)
	{
		if (EraColor != EraColor.Transparent)
		{
			sb.Append(' ').Append(name).Append("='").Append(HtmlManager.GetColorToString(EraColor)).Append('\'');
		}
	}
	public static void AddColorParam4(StringBuilder sb, string name, EraColor[] colors)
	{
		if (colors != null)
		{
			sb.Append(' ').Append(name).Append("='");
