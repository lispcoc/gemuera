...
internal sealed class Shape
{
	...
	{
		public static void DrawBorder(Graphics graph, Rectangle rect, int[] border, int[] radius, EraColor[] colors, EraColor bgColor)
		{
		...
		static void drawNormalCornerBorderLeftTop(Graphics graph, Rectangle rect, int[] border, EraColor bgColor, EraColor[] colors, bool flipX, bool flipY)
		{
			var backUp = graph.SmoothingMode;
			graph.SmoothingMode = SmoothingMode.AntiAlias;
			if (bgColor != EraColor.Transparent)
			{
				// 1/4の背景を描画
			...
			{
				// 左枠の半分
				EraColor EraColor = colors == null ? Config.Config.ForeColor : colors[flipX ? Direction.Right : Direction.Left];
				if (EraColor != EraColor.Transparent)
					using (var path = new GraphicsPath())
					{
						...
						using (var brush = new SolidBrush(EraColor))
						{
			...
			{
				// 上枠の半分
				EraColor EraColor = colors == null ? Config.Config.ForeColor : colors[flipY ? Direction.Bottom : Direction.Top];
				if (EraColor != EraColor.Transparent)
					using (var path = new GraphicsPath())
					{
						path.AddPolygon(new Point[] {
		...
		static void drawRoundedCornerBorderLeftTop(Graphics graph, Rectangle rect, int[] border, int[] radius, EraColor bgColor, EraColor[] colors, bool flipX, bool flipY)
		{
			var backUp = graph.SmoothingMode;
			graph.SmoothingMode = SmoothingMode.AntiAlias;
