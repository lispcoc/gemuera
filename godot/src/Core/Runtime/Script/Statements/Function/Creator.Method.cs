...
internal static partial class FunctionMethodCreator
{
	...
	/// </summary>
	private static EraColor ReadColor(string Name, ExpressionMediator exm, List<AExpression> arguments, int argNo)
	{
		...
		return EraColor.FromArgb((int)(c64 >> 24) & 0xFF, (int)(c64 >> 16) & 0xFF, (int)(c64 >> 8) & 0xFF, (int)c64 & 0xFF);
	}
		...
		public override long GetIntValue(ExpressionMediator exm, List<AExpression> arguments)
		{
			...
			switch (Name)
			{
				...
				case "GGETBRUSH":
					...
					return b.EraColor.ToArgb() & 0xffffffffL;
					#endregion
			}
	...
	public sealed class GraphicsGetColorMethod : FunctionMethod
	{
		...
		public override long GetIntValue(ExpressionMediator exm, List<AExpression> arguments)
		{
			...
			if (p.X < 0 || p.X >= g.Width || p.X < 0 || p.Y >= g.Height)
				return -1;
			EraColor c = g.GGetColor(p.X, p.Y);
			//EraColor.ToArgb()はInt32の負の値をとることがあり、Int64にうまく変換できない?（と思ったが気のせいだった
	...
	public sealed class GraphicsSetColorMethod : FunctionMethod
	{
		...
		public override long GetIntValue(ExpressionMediator exm, List<AExpression> arguments)
		{
			...
			if (!g.IsCreated)
				return 0;
			EraColor c = ReadColor(Name, exm, arguments, 1);
	...
	public sealed class SpriteGetColorMethod : FunctionMethod
	{
		...
		public override long GetIntValue(ExpressionMediator exm, List<AExpression> arguments)
		{
			...
			if (p.Y < 0 || p.Y >= img.DestBaseSize.Height)
				return -1;
			EraColor c = img.SpriteGetColor(p.X, p.Y);
			//EraColor.ToArgb()はInt32の負の値をとることがあり、Int64にうまく変換できない？（と思ったが気のせいだった
